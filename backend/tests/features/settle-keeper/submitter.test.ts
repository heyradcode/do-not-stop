...
describe('createSubmitter', () => {
    it('sends the settle tx when simulation succeeds', async () => {
        ...
        await submitter.submit('settleBreed', 1n);

        expect(publicClient.simulateContract).toHaveBeenCalledWith(
            expect.objectContaining({ functionName: 'settleBreed', args: [1n] }),
        );
        expect(walletClient.writeContract).toHaveBeenCalledWith(
            expect.objectContaining({ functionName: 'settleBreed', args: [1n], gas: 800_000n }),
        );
    ...
    it('processes submissions strictly in call order (one in flight at a time)', async () => {
        ...
        const first = submitter.submit('settleBreed', 1n);
        const second = submitter.submit('settleBreed', 2n);
