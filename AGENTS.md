...
- `MUST NOT` change the on-chain combat ports. `contracts/ethereum/src/CombatSim.sol` and Solana's `combat.rs` are **frozen** as of §L Phase 6 (see `docs/plan-backend-battle-architecture.md`). Every battle they ever settled is a permanent on-chain record, and those records have to stay replayable forever, so editing either one silently rewrites history rather than fixing anything. A bug found in them is fixed forward in the live ports below, under a new `rulesetVersion`, never by patching the frozen ones.
...
- Golden test vectors (`contracts/test-vectors/{battle,xp}.json`), run by Hardhat, Anchor, `indexer-go`'s `combat_golden_test.go`, and `@cryptopets/protocol`'s `tests/combat/goldenVectors.test.ts` (Vitest), are the cross-language enforcement for combat-simulator parity. All four suites keep running after the freeze: the two frozen ports prove the vectors still describe what really settled on chain, and the two live ports prove they have not drifted from it.
