...
Branch: `feat/inventory-item-nfts`.
...
3. **`Ruleset` gains `itemCatalogHash`**, a single 32-byte field over the canonical encoding
   of every combat-affecting `ItemDefinition`, sorted by id. A rebalance then changes
   `rulesetHash`, which is what invalidates outstanding defence authorizations by design.
...
- [ ] **1.1 `ItemCore.sol` core.** ERC-1155 behind UUPS with `OwnableUpgradeable`, matching
...
- [ ] **1.2 Equip and unequip.** `equip(petId, slot, itemType)` transfers one unit into the
      ...
      Verify: `pnpm --prefix contracts/ethereum compile`.
- [ ] **1.3 `test/ItemCore.test.ts`.** Mint, burn, equip escrow, unequip return, double-equip
...
- [ ] **1.4 Deployment wiring.** Add `ItemCore` impl plus `ERC1967Proxy` to
...
- [ ] **1.5 Subgraph.** `ItemBalance` and `PetEquipment` entities in `subgraph/schema.graphql`,
...
- [ ] **1.6 `indexer-go` ingest.** `ItemUpdate` and `EquipmentUpdate` in `internal/indexer`,
...
- [ ] **1.7 `indexer-go` writes.** Version-guarded batch upserts in `internal/store/writer.go`
...
- [ ] **2.1 Prisma models and migration.** `ItemDefinition` (backend-managed catalog),
...
- [ ] **2.2 Catalog seed.** A checked-in JSON catalog plus a loader, so item content is data
      rather than code. Covers all three shipping categories.
      Verify: seed script runs against a local database.
- [ ] **2.3 Read surface.** `backend/src/features/inventory/` laid out like `features/chat`
...
- [ ] **2.4 GraphQL reads.** `inventory(chain, owner)` and `itemCatalog` in
      ...
      Verify: `pnpm --filter backend test`.
- [ ] **2.5 Write surface.** `useItem` (apply the consumable effect to `pet_battle_progress`,
      ...
      Verify: `pnpm --filter backend test`.
- [ ] **2.6 Battle drops.** The battle worker writes `ItemEntitlement` rows in the same
...
- [ ] **3.1 Shared hooks.** `shared/src/hooks/inventory/` (`useInventory`, `useItemCatalog`,
...
- [ ] **3.2 `InventoryAdapter`.** A new chain-blind interface and `useInventoryAdapter` in
...
- [ ] **3.3 Inventory page.** `frontend/src/pages/inventory/index.tsx` plus router and sidebar
...
- [ ] **4.1 Snapshot schema v2.** `PetSnapshot.equipment: EquipEntry[]`
...
- [ ] **4.2 Version-aware snapshot encoder.** `encodeBattleSnapshot` currently always writes
...
- [ ] **4.3 Ruleset v2.** `Ruleset.itemCatalogHash`, `SCHEMA_VERSIONS.ruleset` 1 to 2,
      `RULESET_KEYS` / `serializeRuleset` / `parseRulesetBundle`'s unknown-key rejection all
      updated, and the catalog itself published in the bundle so replay works offline.
      Verify: `pnpm --filter @cryptopets/protocol test`.
- [ ] **4.4 Engine modifiers.** `simulate()` takes per-pet modifiers, applied after `extract`
...
- [ ] **4.5 New golden vectors.** Equipment cases in a new
      `contracts/test-vectors/equipment.json`. `battle.json` and `xp.json` are not edited.
      Verify: `pnpm --filter @cryptopets/protocol test`.
- [ ] **4.6 Go port, same commit.** `combat.PetInputs` gains the modifier, `Simulate`,
...
- [ ] **4.7 Backend wiring.** `snapshot.builder.ts` resolves equipment from the indexed
      ...
      Verify: `pnpm --filter backend test`.
- [ ] **4.8 Verifier.** `checks/combatReplay.ts` and `ruleset.ts` handle both snapshot
...
Bumping `ENGINE_VERSION` and adding `itemCatalogHash` changes `rulesetHash` for every battle,
