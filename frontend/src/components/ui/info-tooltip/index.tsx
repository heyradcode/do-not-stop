 ...
 * keyboard, and this answers a question ("what does this actually do?") rather than labelling
 * a control — so it opens on click, closes on Escape or an outside click, and is announced
 * through `aria-expanded` / `aria-controls` like any other disclosure.
 *
 * The panel is portalled to `document.body` and positioned `fixed`. That is not decoration:
 * it is opened from inside an inventory slot that sets `overflow: hidden`, so a panel
 * rendered in place would be clipped to the tile and mostly invisible. Portalling escapes the
 * clip; `fixed` then keeps it aligned to the button without needing every ancestor to be a
 * positioning context.
 *
 * It closes rather than follows on scroll. A fixed panel anchored to coordinates taken once
 * drifts away from its button as the page moves, and recomputing every frame is a lot of
 * machinery for a panel whose natural lifetime is "read it, dismiss it".
 */
