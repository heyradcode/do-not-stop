...
contract ZombieHelper is ZombieFeeding {
    ...
    modifier onlyOwnerOf(uint256 _zombieId) {
        require(_zombieId > 0 && _zombieId <= getTotalZombies(), "Zombie doesn't exist");
        _;
    }
     ...
     */
    function getZombiesByOwner(address _owner) public view virtual returns (uint256[] memory) {
        uint256[] memory result = new uint256[](ownerZombieCount[_owner]);
        uint256 counter = 0;
        
        for (uint256 i = 1; i <= getTotalZombies(); i++) {
            // Check if zombie exists by checking if it has a name (non-empty)
            if (bytes(zombies[i].name).length > 0) {
                result[counter] = i;
                counter++;
            }
        }
     ...
     */
    function getZombieStats(uint256 _zombieId) public view returns (uint32, uint16, uint16, uint8) {
