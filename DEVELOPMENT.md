...
Solana WebSocket push) into `pet_roster`, and serves pet-state reads and win
estimates to the backend over gRPC. It no longer touches `battle_history`, which
the backend writes from signed receipts. Build/test/runbook:
