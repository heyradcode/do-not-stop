...
contract ZombieAttack is ZombieHelper {
    ...
    function attack(uint256 _zombieId, uint256 _targetId) public {
        ...
        require(
            _targetId > 0 && _targetId <= getTotalZombiesCount(),
        ...
        require(
            block.timestamp >= enemyZombie.readyTime,
            "Enemy zombie not ready"
        );
        ...
        if (won) {
            ...
            _createZombie(
                "BattleVictory",
                enemyZombie.dna,
                _calculateRarity(enemyZombie.dna)
            );
        } else {
            myZombie.lossCount++;
    ...
    function getBattleStats(
        ...
        uint256 winRate = totalBattles > 0
            ? (zombie.winCount * 100) / totalBattles
            : 0;
