...
contract ZombieOwnership is ZombieAttack, ERC721, ReentrancyGuard {
     ...
     */
    function getZombieMetadata(uint256 tokenId) public view returns (string memory) {
        require(_ownerOf(tokenId) != address(0), "Token does not exist");
        
        return string(abi.encodePacked(
            '{"name":"Zombie #', _toString(tokenId), '"}'
        ));
    }
     ...
     */
    function _createZombie(string memory _name, uint256 _dna, uint8 _rarity) internal override returns (uint256) {
     ...
     */
    function getZombiesByOwner(address owner) public view override returns (uint256[] memory) {
