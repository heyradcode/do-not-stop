...
- CI coverage workflow (`.github/workflows/coverage.yml`) runs frontend/backend/shared vitest coverage on every PR and posts a combined comment. The verifier workflow (`.github/workflows/verifier.yml`) replays a committed receipt corpus through the standalone verifier, asserts a tampered corpus is rejected, and replays a second corpus spanning both chains' signing keys.
- **No CI job builds Rust**, so a Solana program's own tests never run in CI. The four places that transcribe those programs by hand — the reward-leaf vectors in `leaf.rs`, indexer-go's IDL, the backend's `RegistryState` offsets, and `shared`'s PDA seeds and memcmp offsets — are each checked against the Rust source by a test in the consuming package. See CLAUDE.md's table. A new hand-transcription `MUST` add an entry to it; the alternative is a constant that is only wrong in production.
